...
static int64_t days_from_2000( const mu_date_t *p_date );
static void date_from_days( int64_t total_days, mu_date_t *p_date );
static uint32_t secs_from_midnight( const mu_time_t *p_time );
static void time_from_secs( uint32_t secs, mu_time_t *p_time );
static int64_t days_min( void );
static int64_t days_max( void );

static const uint8_t s_month_days[12] =
{
    31, 28, 31, 30, 31, 30,
    31, 31, 30, 31, 30, 31,
};
...
static uint8_t days_in_month( uint16_t year, uint8_t month )
{
    ...
    if( month == 2U && is_leap_year( year ) == true )
    {
        return 29;
    }

    return s_month_days[month - 1U];
}

static uint16_t year_min( void )
{
...
static int64_t days_from_2000( const mu_date_t *p_date )
{
    ...
    if( p_date->year >= 2000U )
    {
        ...
        {
            days += days_in_month( p_date->year, m );
        }
...
static void date_from_days( int64_t total_days, mu_date_t *p_date )
{
    ...
    if( total_days >= 0 )
    {
        ...
        while( true )
        {
            int64_t year_days = is_leap_year( y ) ? 366 : 365;
    ...
    else
    {
        ...
        p_date->day = days_in_month( y, m ) - ( uint8_t )total_days;
    }
...
bool mu_date_is_valid( const mu_date_t *p_date )
{
    ...
    if( p_date->day < 1U || p_date->day > days_in_month( p_date->year, p_date->month ) )
    {
        return false;
    }

    if( p_date->year < year_min() || p_date->year > year_max() )
    {
        return false;
    }
...
uint16_t mu_date_day_of_year( const mu_date_t *p_date )
{
    ...
    for( m = 1; m < p_date->month; m++ )
    {
        doy += days_in_month( p_date->year, m );
    }
...
bool mu_datetime_to_timestamp_ex( const mu_datetime_t *p_dt, uint32_t *p_timestamp )
{
    ...
    days = days_from_2000( &p_dt->date );
    secs = days * 86400 + ( int64_t )secs_from_midnight( &p_dt->time );
...
bool mu_timestamp_to_datetime( uint32_t timestamp, mu_datetime_t *p_dt )
{
    ...
    date_from_days( days, &p_dt->date );
    time_from_secs( secs, &p_dt->time );
...
bool mu_date_add_days( mu_date_t *p_date, int32_t days )
{
    ...
    d = days_from_2000( p_date ) + days;

    if( d < days_min() || d > days_max() )
    {
        return false;
    }

    date_from_days( d, p_date );
...
int32_t mu_date_diff_days( const mu_date_t *p_date1, const mu_date_t *p_date2 )
{
    ...
    d1 = days_from_2000( p_date1 );
    d2 = days_from_2000( p_date2 );
    diff = d1 - d2;
...
bool mu_time_add_seconds( mu_time_t *p_time, int32_t seconds )
{
    ...
    total = ( int32_t )secs_from_midnight( p_time ) + seconds;
    ...
    time_from_secs( ( uint32_t )total, p_time );
